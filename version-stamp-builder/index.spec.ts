...
describe('Version Stamp Builder', () => {
    ...
    it('should write src/environments/version-info.ts file', async () => {
        // Create a logger that keeps an array of all messages that were logged.
        const logger = new logging.Logger('');
        const logs: string[] = [];
        logger.subscribe(ev => logs.push(ev.message));

        const run = await architect.scheduleBuilder('angular-version-stamp:generate', {}, {logger});
        ...
        expect(fs.pathExistsSync(join(__dirname, 'src', 'environments', 'version-info.ts'))).toBe(true);
    });
